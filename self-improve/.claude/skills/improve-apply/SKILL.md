...
- 各採用項目には「**期待する効果**」を1行添える（どのシグナルが消えるはずか。
  例: `期待する効果: /foo 実行時の「そうじゃなくて」訂正が出なくなる`）。
  次回 `improve-scan` の**効果検証（再発突合）**がこの行を突合キーに使う。
...
`improve-scan` の効果検証が backlog に積んだ「効果なし: <改善> → 巻き戻し or 再改善」候補は、
通常候補と同じく**1件ずつ承認**で扱う:

- **巻き戻し** — 適用時に残した `.bak` を復元（JSON マージ系は記録済みエントリの remove）。
  巻き戻したら採用履歴に「巻き戻し済み（効果なし）」を追記し、同じ改善を再提案しない。
- **再改善** — 元の改善を修正して再適用（期待する効果も書き直す）。
- 巻き戻しを却下した場合は「合法的例外」として記録する（再発していても意図して残す判断）。

棚卸し候補（未使用スキル）は **(a) `.claude/skills-archive/` への移動**（`.bak` 相当として移動元
パスを記録し、戻せるようにする）か **(b) description のトリガー改善** として適用する。
