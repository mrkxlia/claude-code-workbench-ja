...
## 棚卸しと効果検証（v0.3.0・先行研究の取り込み）

[RESEARCH.md](RESEARCH.md) §4 の取り込み候補のうち2つを導入しています:

- **棚卸し（鮮度管理）** — improve-scan が、スキャン範囲で一度も起動されていない自作スキルを
  「アーカイブ or トリガー改善」の2択候補として backlog に挙げる（「増やす」だけでなく「減らす」
  方向の改善。UniM0cha 式 stale 管理の承認制版）。
- **効果検証（再発突合）** — improve-apply が採用履歴に「期待する効果」を1行残し、次回の
  improve-scan が適用後セッションでの同種シグナル再発を突合。再発していれば「効果なし →
  巻き戻し or 再改善」候補を挙げる（DGM の実証検証 / PACE の受理判定のローカル版）。
...
自己改善エージェントの学術論文（Reflexion / Voyager / SICA / Darwin Gödel Machine 等）と
Claude Code 向け OSS 実装の比較、本セクションの設計との対応づけは [RESEARCH.md](RESEARCH.md) に
まとめています。
