...
## 4. 取り込み候補のアイデア（提案のみ・未実装）

先行事例から self-improve に将来取り込む価値がありそうなもの:

1. **スキル鮮度管理**（UniM0cha 式）— improve-scan が生成/改善したスキルに最終使用日を記録し、
   長期未使用スキルを「アーカイブ候補」として backlog に挙げる。「増やす」だけでなく「減らす」
   方向の自己改善
2. **スキル利用実績トラッキング**（singularity-claude 式）— 適用済み改善が実際に使われたか・
   効いたかをトランスクリプトから追跡し、効果のなかった改善の巻き戻し候補化に使う
3. **適用前後の比較**（PACE / DGM 式）— improve-apply の適用記録に「期待した効果」を残し、
