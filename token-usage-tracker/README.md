...
# 1) リポジトリを取得（このツールだけ欲しいので浅いクローンでOK）
git clone --depth 1 https://github.com/mrkxlia/claude-code-workbench-ja.git
cd claude-code-workbench-ja/token-usage-tracker
...
> このツールは `claude-code-workbench-ja` リポジトリ内の `token-usage-tracker/` ディレクトリに
> 入っています。リポジトリ全体は不要なので上記の浅いクローンで十分です。
