...
| # | エージェント | 役割 | 許可ツール | 書き込み範囲 | 主な成果物 |
|---|-------------|------|-----------|-------------|-----------|
| 1 | `codebase-researcher` | 作る前にコードをマッピングする | Read, Grep, Glob | なし | 調査レポート（research.md） |
| 2 | `story-writer` | アイデアを受け入れ基準つきストーリーにする | Read | なし | ユーザーストーリー（story.md）🛑承認1 |
| 3 | `spec-writer` | ストーリーを技術ブリーフにする | Read, Grep, Glob | なし | 技術ブリーフ（brief.md）🛑承認2 |
| 4 | `backend-builder` | API・サービス・ジョブ・ユニットテスト | Read, Grep, Glob, Edit, Write, Bash | バックエンドのフォルダのみ | 実装 + API契約（api-contract.md） |
| 5 | `frontend-builder` | コンポーネント・ページ・フック・UIテスト | Read, Grep, Glob, Edit, Write, Bash | フロントエンドのフォルダのみ | 実装 + サマリー |
| 6 | `test-verifier` | ストーリーに対する受け入れテスト | Read, Grep, Glob, Edit, Write, Bash | テストファイルのみ | 受け入れテスト + 検証レポート |
| 7 | `implementation-validator` | 実装とストーリー/ブリーフのギャップ報告 | Read, Grep, Glob | なし | Critical/Important/Minor レポート 🛑承認3 |
...
cp -r <このリポジトリ>/software-factory/.claude/skills/* .claude/skills/
```
...
- **`tools` 制限はツール単位であり、フォルダ単位ではありません。** 「backend-builder はバックエンドのフォルダのみ」という境界は、エージェント定義のプロンプトによる制約です。実用上はよく守られますが、厳密に強制したい場合は Edit/Write のパスを検査する PreToolUse フックを追加するのが発展課題です
