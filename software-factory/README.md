...
**30秒でわかる software-factory:**
`/feature-factory <機能の説明>` と打つと、7つの専門エージェントが順番に起動し、
調査レポート → ユーザーストーリー → 技術ブリーフ → 実装 → テスト → 最終検証まで自走します。
あなたの仕事は途中3回の「承認」だけ。途中経過はすべて `docs/factory/<機能名>/` にファイルとして
残るので、セッションが切れても続きから再開できます。

---

## なぜ「工場」にするのか — 問題解決への影響
...
flowchart LR
    ...
    subgraph factory["✅ 工場: 役割分離 + 早期チェックポイント"]
        direction TB
...
```mermaid
flowchart TB
    req(["あなた: /feature-factory<br/>「7日以上未払いの請求書に支払いリマインダーを作って」"]) --> p1
...
## 成果物と関係性 — `docs/factory/<slug>/` を中心としたデータの流れ
...
flowchart TB
    ...
    subgraph slug["docs/factory/＜slug＞/ — 機能ごとの成果物"]
    ...
    notes -->|"Decisions / Deferred を回収"| learn["docs/factory/LEARNINGS.md<br/>ルール候補の蓄積"]
    learn -->|"🛑3 で承認されたら昇格"| cmd["CLAUDE.md のルール"]
    notes -.->|"再開時に Status ブロックを読む"| resume["/feature-factory 再開 ＜slug＞"]
...
- **`status.md`** — 工場の**進行管理**（フェーズ・承認・差し戻しカウンタ）。中断・再開の正
...
flowchart TD
    q0{"やりたいことは？"}
    q0 -->|"工場をまだ導入していない"| setup["/factory-setup<br/>対象リポジトリに一式を自動導入"]
    q0 -->|"機能を end-to-end で開発"| q1{"対象コードに<br/>仕様書はある？"}
    q1 -->|"ない（レガシー）"| se["/spec-extract<br/>現状の挙動を SPEC.md に固定"]
    se --> ff["/feature-factory ＜機能の説明＞<br/>7エージェント + 3チェックポイント"]
    q1 -->|"ある / 新規開発"| ff
    q0 -->|"工場を通すほどではない<br/>小さな実装・修正"| bwt["/build-with-tests ＜タスク＞<br/>既存パターン確認 + テスト並行"]
    q0 -->|"実装メモを手動で<br/>残したい・更新したい"| nt["/notes<br/>（通常はビルダーが自動記録）"]
    q0 -->|"工場の運用実績から<br/>定義を改善したい"| fi["/factory-improve<br/>失敗シグナル検出 + 修正提案"]
    q0 -->|"コード以外の成果物<br/>（図・ドキュメント・レポート）"| tf["task-factory セクションへ<br/>（5エージェント汎用工場）"]
...
| `/spec-extract [対象パス]` | レガシーコードの仕様を逆引きして SPEC.md に固定する（工場導入の前工程） |
| `/notes` | 実装ノートを手動で開始・更新する（工場内ではビルダーが自動記録） |
| `/factory-improve [期間や slug]` | 運用実績から失敗シグナルを検出し、エージェント定義・スキル・CLAUDE.md の改善案を提案・適用する（自己改善ループ） |
| `/factory-setup` | 工場一式を対象リポジトリへ自動導入する |
...
[`implementation-skills/`](../implementation-skills/) セクションのスキルの**工場連携版**です。
工場の空白だった2つを埋めます:

- **notes（実装ノート）** — ビルダー3種が「ブリーフにない判断・逸脱・トレードオフ・
  ハマりどころ・積み残し」を `docs/factory/<slug>/implementation-notes.md` に物証つきで記録します。
  `/feature-factory 再開` は冒頭の Status ブロックを最初に読み、
  最終レビュー（Phase 7）では Decisions / Deferred が LEARNINGS.md のルール候補として回収されます。
  「なぜこう書いたのか」がセッションを跨いで残ります
- **spec-extract（仕様逆引き）** — 仕様書のないレガシーコードに工場を導入する前に
  `/spec-extract` で現状を `SPEC.md` に固定し（全記述に `[確定]/[推定]/[不明]` の確度ラベル）、
  codebase-researcher がそれを一次資料として読みます。レガシー導入の推奨フローは
  **`/spec-extract` → SPEC.md を人間レビュー → `/feature-factory`**

<details>
<summary><b>原本との同期ルール</b>（implementation-skills 原本との差分確認コマンド）</summary>

- **単体で使いたい**（工場を導入しないプロジェクト・単発の実装）→ [`implementation-skills/`](../implementation-skills/) の**原本**をコピーする
- **工場で使う** → このセクションの工場連携版を使う（`/factory-setup` が自動配布します）

工場連携版は「原本の完全コピー + 末尾の `FACTORY-INTEGRATION` マーカー以降に工場連携セクション」
という構造です。**原本を更新したら、マーカーより上を新しい原本でまるごと差し替えてください。**
一致確認はこのコマンドでできます:

```bash
orig=implementation-skills/.claude/skills/notes/SKILL.md
diff <(head -n "$(wc -l < "$orig")" software-factory/.claude/skills/notes/SKILL.md) "$orig"
# spec-extract も同様。出力が空なら一致
```
...
└── .claude/
    ...
    │   ├── notes/SKILL.md                   # 実装ノート（implementation-skills 由来の工場連携版）
    │   ├── spec-extract/SKILL.md            # 仕様逆引き（implementation-skills 由来の工場連携版）
    │   ├── factory-improve/SKILL.md         # 自己改善ループ（失敗シグナル検出 → 定義の改善提案）
...
`factory-setup` が対象リポジトリへコピー&カスタマイズします。
...
どの方式でも最後は `/factory-setup`（または手動コピー）が対象リポジトリに
...
```
/software-factory:factory-setup
```

プラグインのスキルは `/software-factory:factory-setup` のように**プラグイン名の名前空間付き**で
呼び出します。factory-setup がプロジェクトへスキルをコピーした後は、プロジェクト側が優先される
ため、短い `/feature-factory` などがそのまま使えます。
プラグインの更新は `/plugin update software-factory@workbench-ja` で取り込めます。

<details>
<summary><b>方式B: git clone + factory-setup</b>／<b>factory-setup がやること</b></summary>

### 方式B: git clone + factory-setup（2コマンド）

プラグインを使わない場合は、clone して `factory-setup` をパーソナルスキルとして
...
cp -r <このリポジトリ>/software-factory/.claude/skills/clarify .claude/skills/
cp -r <このリポジトリ>/software-factory/.claude/skills/build-with-tests .claude/skills/
cp -r <このリポジトリ>/software-factory/.claude/skills/notes .claude/skills/
cp -r <このリポジトリ>/software-factory/.claude/skills/spec-extract .claude/skills/
cp -r <このリポジトリ>/software-factory/.claude/skills/factory-improve .claude/skills/
```
...
cp <このリポジトリ>/software-factory/.claude/hooks/guard-builder-writes.sh .claude/hooks/
...
なお「上記に加えて〜（差し替え対象外）」とある `docs/factory/<slug>/implementation-notes.md` の行は
...
- ビルダーが実装中に気づいた「このルールがあれば助かった」はサマリー経由で
  `docs/factory/LEARNINGS.md` に自動で蓄積されます
...
### 4. `/factory-improve` で自己改善ループを回す

LEARNINGS.md の回収が「CLAUDE.md のルール候補」止まりなのに対し、`/factory-improve` は
...
0 9 * * 1-5 cd /path/to/project && claude -p "/factory-improve 直近1日 — 提案の提示までを行い、適用はせず docs/factory/improve-report.md に提案を書き出して終了して" --permission-mode acceptEdits >> ~/factory-improve.log 2>&1
```

レポートを朝に確認し、採用する提案だけ対話セッションの `/factory-improve` で適用する運用が
...
- **並列実行時のフックが守るのは「共有ファイル衝突」だけで、「グループ境界の越境」ではありません。** 同梱の [`guard-builder-writes.sh`](.claude/hooks/guard-builder-writes.sh) は、並列フェーズ中（`docs/factory/<slug>/.parallel-active` が存在）に schema/マイグレーション/`package.json`/型バレル等の共有ファイルへ書き込もうとすると `ask` で確認します。一方「グループAのビルダーがグループBのサブツリーへ書く」越境はフックでは検出できない（brief の所有宣言がフックに渡らないため）ので、これは brief の所有パス宣言＋工場長の越境チェックで守ります。共有ファイル禁止リスト（`SHARED_PATTERNS`）は自分のスタックに合わせて調整してください
...
ソフトウェア工場は「調査 → ストーリー → ブリーフ → 実装 → 検証」を**既定では逐次**に進める
パイプラインで、各工程は前工程の成果物（`docs/factory/<slug>/` のファイル）だけを入力に動きます。
...
## コラム: 逐次パイプライン工場と「並列ループエージェント」の使い分け
...
| | 逐次パイプライン工場（本テンプレート） | 並列ループエージェント |
|---|---|---|
...
後者は「ブリーフ承認の前で設計ミスを捕まえる」という工場の価値と衝突するためです。
...
- [Claude Code のスキルが毎日勝手に改善されていく仕組みを作った（SonicGarden）](https://zenn.dev/sonicgarden/articles/claude-code-self-improving-loop) — `/factory-improve` の着想元
