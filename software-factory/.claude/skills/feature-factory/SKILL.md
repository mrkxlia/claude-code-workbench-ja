...
修正指示があれば story-writer を修正内容つきで再起動し、再びここで停止する（再提示も step 3 と同じ要点サマリー＋全文の形式で行う）。
