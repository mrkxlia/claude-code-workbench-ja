...
<!-- FACTORY-INTEGRATION: この行より上は implementation-skills/.claude/skills/notes/SKILL.md の原本と同一に保つ。
...
工場で使うとき、上記の原本ルールに以下が**優先して**加わる:

### ファイルの置き場所（原本の「Where the file lives」より優先）

- 工場の機能開発では **1機能 = 1ファイル**: `docs/factory/<slug>/implementation-notes.md`
- feature-factory のオーケストレーターやエージェント定義から記録先パスを指示された場合は
  常にそのパスへ書く。`docs/factory/<slug>/` が存在する作業中は、リポジトリルートに
  `implementation-notes.md` を新規作成しない
- 工場の外の作業（単発の修正など）では原本どおりの置き場所でよい

### status.md と混ぜない

- `docs/factory/<slug>/status.md` は工場の**進行管理**（フェーズ・承認・差し戻しカウンタ）
...
- **再開時**: `/feature-factory 再開 <slug>` は status.md と合わせてこのファイルの
  Status ブロックを最初に読む
- **Phase 7（最終検証）後**: Decisions / Deferred のうち他機能にも一般化できるものは
  `docs/factory/LEARNINGS.md` の候補としてオーケストレーターが回収し、
  チェックポイント3でユーザーに提示する
