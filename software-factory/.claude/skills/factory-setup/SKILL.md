...
このスキル（factory-setup）が `<どこか>/.claude/skills/factory-setup/` という配置にあり、
その **3階層上**のディレクトリに `.claude/agents/` と `CLAUDE.md` が存在する場合、
そこがテンプレート一式である。`$SRC=<3階層上のディレクトリ>` として Step 2 へ進む。
clone もユーザーへの質問も不要。

該当するのは次の2ケース:
- **プラグインとしてインストールされている**（`/plugin install software-factory@workbench-ja`）—
  プラグインのキャッシュに software-factory 一式が入っており、このスキルはその中にいる
- workbench リポジトリの clone 内の `software-factory/.claude/skills/factory-setup/` から
  直接読み込まれている

`~/.claude/skills/factory-setup/` にパーソナルスキルとして単体コピーされている場合は
...
2. `backend-builder.md` / `frontend-builder.md` / `test-verifier.md` の
   ...
   **「上記に加えて〜（差し替え対象外）」以降の `docs/factory/<slug>/implementation-notes.md` の
...
プロジェクト側のスキルが優先されるため衝突せず、後でプラグインを無効化しても工場が動き続ける。

`notes/` と `spec-extract/` は implementation-skills 由来の工場連携版。対象リポジトリに
既に同名スキル（原本を単体導入済みのケース）がある場合は上書きせず、スキップして
「工場連携版に差し替えるか」をユーザーに確認する。
