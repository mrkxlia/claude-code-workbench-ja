...
| `evals/` | 主要スキル（task-brief / verify-fresh / long-run / review-panel / adoption-review）の「期待挙動」シナリオ18件。Sonnet 5 と Opus 5 のパリティを実測する物差し。`claude plugin eval` が early access のため Markdown 形式（ゲートが開いたら `case.yaml` へ機械化する）。追補ルール14 の分岐が実効を持つかを verify-fresh S-3/S-4 で検出する |
