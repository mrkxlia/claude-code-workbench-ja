...
Track B の共有仕様。**原本 `software-pipeline/.claude/{skills/feature-pipeline,agents,CLAUDE.md}` を一次根拠**に起こした
...
- **Claude Code（原本）**: `software-pipeline/.claude/{skills,agents}`（参照元）。
