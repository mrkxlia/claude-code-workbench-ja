...
Track B の共有仕様。**原本 `plugins/task-pipeline/.claude/{skills/task-pipeline,agents,CLAUDE.md}` を一次根拠**に起こした
...
- **Claude Code（原本）**: `plugins/task-pipeline/.claude/{skills,agents}`（参照元）。
