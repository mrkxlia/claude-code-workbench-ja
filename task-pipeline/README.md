...
（[`block-secrets-commit.sh`](../software-pipeline/.claude/hooks/block-secrets-commit.sh)）を
併用するのがおすすめです。
...
cp -r <このリポジトリ>/task-pipeline/.claude/skills/task-pipeline-setup ~/.claude/skills/
```
...
cp <このリポジトリ>/task-pipeline/.claude/agents/*.md .claude/agents/
```
...
```bash
mkdir -p .claude/skills
cp -r <このリポジトリ>/task-pipeline/.claude/skills/task-pipeline .claude/skills/
cp -r <このリポジトリ>/task-pipeline/.claude/skills/clarify .claude/skills/
```
...
cp <このリポジトリ>/task-pipeline/.claude/hooks/guard-deliverable-writes.sh .claude/hooks/
...
```bash
cp <このリポジトリ>/task-pipeline/.claude/settings.json .claude/settings.json
```
