...
1. **個人の運用ルールを先に整える**（Claude Code のユーザー設定に一度入れれば全プロジェクトで効く）— `model-setup` を導入（9ルール＋`task-brief`／`backlog-loop`／`pr-merge`）。
...
> フックは一覧の5プラグインのみが持ちます。他のプラグイン（model-setup・ai-peer 等）はスキルのみで完結し、常駐フックはありません。
...
- **model-setup** — 導入すると `/task-brief`（着手前にタスク仕様を一括ブリーフ化）・
  `/backlog-loop`（backlog.md 駆動の定型ループ）・`/pr-merge`（PR作成〜マージ〜後片付け、git/gh
  専用）が使えます。モデル・effort 選定ガイド（MODEL-GUIDE.md）や CLAUDE.md テンプレート（9ルール）
  も同梱。詳しくは [model-setup/README.md](model-setup/) を参照。
...
# model-setup — Sonnet 運用ルールをグローバル CLAUDE.md に追記 + スキル2種（pr-merge は git 専用のため必要な環境のみ）
cat /tmp/workbench/model-setup/CLAUDE.md >> ~/.claude/CLAUDE.md
cp -r /tmp/workbench/model-setup/.claude/skills/task-brief /tmp/workbench/model-setup/.claude/skills/backlog-loop ~/.claude/skills/
```
...
| Sonnet/Haiku しか使えない環境で精度を底上げしたい | **model-setup** | 上位モデルの「振る舞い」を CLAUDE.md に常設化（9ルール）＋モデル/effortガイド |
...
Sonnet 5 / Opus 4.8 運用テンプレート。完了条件の事前定義・検証つき完了報告・確信度の明示・
スコープ厳守・網羅レビューなど、上位モデルの「振る舞い」を常設化する9つの行動ルールに加え、
最初のターンでタスク仕様をブリーフ化する **task-brief**、backlog.md 駆動の定型ループ
**backlog-loop**、PR作成〜マージ〜後片付けまでの **pr-merge**（git/gh 専用）の3スキル、
モデル・effort 選定ガイド（`MODEL-GUIDE.md`）、私用/会社PC 別の settings サンプルを収録して
います。**プラグイン1コマンドで導入可能**（上の「導入方法」参照）。プロンプト側の型は既存 OSS
（severity1/claude-code-prompt-improver）を README で紹介しています。
