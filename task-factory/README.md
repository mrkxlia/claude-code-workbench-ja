...
**30秒でわかる task-factory:**
`/task-factory <作りたい成果物>` と打つと、5つの専門エージェントが順番に起動し、
調査 → 要件 → ブリーフ → 成果物作成 → レビューまで自走します。
あなたの仕事は途中3回の「承認」だけ。途中経過は `docs/taskfactory/<slug>/` にファイルとして
残るので、セッションが切れても `/task-factory 再開 <slug>` で続きから再開できます。
...
迷ったら: **コード変更が必要なら（実装・修正・実装ノート・仕様逆引きを含む）→ software-factory、
コード以外の成果物（図・ドキュメント・レポート）→ task-factory**。
...
```mermaid
flowchart TB
    req(["あなた: /task-factory<br/>「認証システムのアーキテクチャ図を drawio で作って」"]) --> p1
...
## データの流れ — `docs/taskfactory/<slug>/` を中心に
...
flowchart TB
    ...
    subgraph slug["docs/taskfactory/＜slug＞/ — タスクごとの中間成果物"]
    ...
    out --> dr
    db -->|"提案を蓄積"| learn["docs/taskfactory/LEARNINGS.md<br/>ルール候補"]
...
software-factory と同じ思想で、次の2つを取り入れています。

- **要件ヒアリングの徹底質問（`clarify` スキル）** — Phase 2（要件）・Phase 3（ブリーフ）で、
  writer を起動する前に工場長が `clarify` で要件・構成を**一問ずつ**（各問に推奨回答つき）詰めます。
...
- **並列実行グループ** — brief の「並列実行プラン」が独立した成果物グループ（出力パスが交わらず
  ...
  実行します。複数の独立した図・レポートを同時に作れます。グループ境界の越境は工場長が守ります
  （フックは出力ディレクトリ外のみ確認）。

逐次パイプライン工場と「並列ループエージェント」方式の使い分けは、
[software-factory のコラム](../software-factory/README.md#コラム-逐次パイプライン工場と並列ループエージェントの使い分け)に
まとめてあります（同じ考え方が task-factory にも当てはまります）。
...
cp -r <このリポジトリ>/task-factory/.claude/skills/clarify .claude/skills/
```
