...
CLAUDE.md・エージェント5種（ビルダーの「担当範囲」も自動差し替え）・スキルをまとめて導入します。
**CLAUDE.md の出力先とビルダーの担当範囲を同じ検出結果から生成する**ため、両者の不一致が
構造的に起きません。
...
<details>
<summary><b>フォールバック: 手動セットアップ（3ステップ）</b> — オフライン環境や、仕組みを理解しながら導入したい場合</summary>
...
- **`tools` 制限はツール単位であり、フォルダ単位ではありません。** 「ビルダーは出力ディレクトリのみ」という境界は、エージェント定義のプロンプトによる制約です。実用上はよく守られますが、厳密に強制したい場合は Edit/Write のパスを検査する PreToolUse フックを追加するのが発展課題です（software-factory の [`block-secrets-commit.sh`](../software-factory/.claude/hooks/block-secrets-commit.sh) が実装の参考になります）
