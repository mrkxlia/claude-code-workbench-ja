...
- [ ] Phase 2: Requirements → requirements.md 保存 → 🛑 要件承認（承認: ／方式: ）
...
工場は research.md / requirements.md / brief.md 等を**書き出す**ため、全体は**通常モード**で動かす。
...
1. Task ツールで `requirements-writer` を起動する。入力: 依頼内容 + research.md の内容
2. 返ってきた要件を `docs/taskfactory/<slug>/requirements.md` に保存する
3. 要件全文をユーザーに提示する

**🛑 チェックポイント1 — 「Plan モードレビューについて」の手順で承認を取る。**
- **(A)**: `EnterPlanMode` →（requirements.md の全文 ＋ 遷移宣言「承認後は brief-writer を起動して作業ブリーフを作り、
  CP2 で必ず停止する」）を計画として提示 → `ExitPlanMode` で承認を得る。承認まで**絶対に Phase 3 に進まない**。
- **(B)**: requirements.md 全文を提示し、明示的な承認（「承認」「OK」「進めて」等）があるまで Phase 3 に進まない。
- 承認後は status.md の「要件承認」に日付と方式（Plan／テキスト）を記録する。
修正指示があれば requirements-writer を修正内容つきで再起動し、再びここで停止する（(A) の場合は再び `EnterPlanMode` に入り直して提示する）。

## Phase 3: Brief（作業ブリーフ）→ 🛑 チェックポイント2

1. Task ツールで `brief-writer` を起動する。
   入力: 承認済み requirements.md + research.md + CLAUDE.md のルール
   （出力ディレクトリ・利用可能なスキル・表記規約）
2. 返ってきたブリーフを `docs/taskfactory/<slug>/brief.md` に保存する
3. ブリーフ全文をユーザーに提示する

**🛑 チェックポイント2 — 「Plan モードレビューについて」の手順で承認を取る。この時点では成果物が1ファイルも作成されていないこと。**
- **(A)**: `EnterPlanMode` →（brief.md の全文 ＋ 遷移宣言「承認後は Phase 4（deliverable-builder）で成果物を作り、
  Phase 5 のレビューを経て CP3 で必ず停止する」）を計画として提示 → `ExitPlanMode` で承認を得る。承認まで**絶対に Phase 4 に進まない**。
- **(B)**: brief.md 全文を提示し、明示的な承認があるまで Phase 4 に進まない。
- 承認後は status.md の「ブリーフ承認」に日付と方式（Plan／テキスト）を記録する。
修正指示があれば brief-writer を再起動し、再びここで停止する（(A) の場合は再び `EnterPlanMode` に入り直して提示する）。
