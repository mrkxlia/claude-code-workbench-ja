...
3. ユーザーがファイルを開かずにレビューできるよう、チャットに次の2つを必ず表示する
...
修正指示があれば requirements-writer を修正内容つきで再起動し、再びここで停止する（再提示も step 3 と同じ要点サマリー＋全文の形式で行う）。
...
3. ユーザーがファイルを開かずにレビューできるよう、チャットに次の2つを必ず表示する
   ...
   - **要点サマリー** — 主要な作業判断・対象成果物・残る論点
   - **全文** — 保存した brief.md の内容を省略・要約せずそのまま提示する

**STOP. ユーザーの明示的な承認があるまで、絶対に Phase 4 に進んではならない。
この時点では成果物が1ファイルも作成されていないこと。**
修正指示があれば brief-writer を再起動し、再びここで停止する（再提示も step 3 と同じ要点サマリー＋全文の形式で行う）。
