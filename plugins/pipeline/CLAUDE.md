...
| エージェント | 役割 | 書き込み範囲 |
|-------------|------|-------------|
| `researcher` | 作る前にコードをマッピングする | なし（読み取り専用） |
| `requirements-writer` | アイデアを受け入れ基準つきストーリーにする | なし（読み取り専用） |
| `brief-writer` | ストーリーを技術ブリーフにする | なし（読み取り専用） |
...
| `final-reviewer` | 実装とストーリー/ブリーフのギャップ報告 | なし（読み取り専用） |
...
- **Inception（要件・設計）** = Phase 1〜3。`researcher`=調査役・`requirements-writer`=要件役・
  `brief-writer`=設計役。ストーリー承認・ブリーフ承認の2つが承認ゲート
...
- **検証ゲート** = Phase 7 の `final-reviewer`（検証役・読み取り専用）と最終レビュー承認。
...
- **調査の網羅性** — researcher は変更に関係する範囲に限り、呼び出し連鎖・依存・既存実装を
