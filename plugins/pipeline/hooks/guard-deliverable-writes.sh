...
#      人間に確認を求める。即ブロックにしないのは、オーケストレーター（メインセッション）の正当な書き込み
...
# pipeline-setup スキル（成果物モード）は、承認済みの出力ディレクトリでここを自動的に書き換える。
# CLAUDE.md の「成果物の種類と出力先」・deliverable-builder の「担当範囲」と一致させること。
ALLOWED_PREFIXES="deliverables/ docs/task-pipeline/ .claude/"  # 書き込みを許可するディレクトリ（スペース区切り）
