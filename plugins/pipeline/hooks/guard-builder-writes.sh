...
#   オーケストレーターの越境チェックで守る。フックは brief の宣言を受け取れないため、所有境界は判定できない。
...
#   mkdir -p docs/pipeline/demo && touch docs/pipeline/demo/.parallel-active
#   echo '{"tool_name":"Write","tool_input":{"file_path":"prisma/schema.prisma"}}' \
...
#     | bash .claude/hooks/guard-builder-writes.sh; echo $?            # → 0
#   rm docs/pipeline/demo/.parallel-active

set -u

# --- 設定（プロジェクトに合わせて差し替えてください）-------------------------
# 並列グループ間で衝突しうる「共有ファイル」のパターン。CLAUDE.md のアーキテクチャ・
# brief-writer の「並列実行プラン（共有/先行逐次変更）」と一致させること。
...
if [ -d "$ROOT_DIR/docs/pipeline" ]; then
  # docs/pipeline/<slug>/.parallel-active のいずれかがあれば並列中とみなす
  for marker in "$ROOT_DIR"/docs/pipeline/*/.parallel-active; do
