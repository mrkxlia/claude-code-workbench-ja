...
SPEC.md を**増分更新**する（新規挙動=新 `F-NN` 追加、変更=同 ID で要件を書き換え＋§9 改訂履歴に
1行、廃止=`[廃止]` 印）。brief の「既存仕様への影響」で宣言された差分がそのまま反映対象。
これで SPEC.md が spec of record として陳腐化しない。
SPEC.md が無いリポジトリでは無理に作らずスキップしてよい（必要なら cc-rsg 等の外部ツールでの
仕様逆引き生成を案内）。
