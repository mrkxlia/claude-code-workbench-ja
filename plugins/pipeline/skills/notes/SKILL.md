...
このファイルは既存コードからの仕様逆引き（cc-rsg 等の外部ツールを使う場合を含む）に
とって一級の物証源。きちんと保たれたノートは、逆引きした仕様の「推定」を「確定」に
変える — エントリをファイル・テストに固定しておくべき理由がもう一つ増える。
...
増分更新**を行う（要件を書き換え、改訂履歴の行を追記する）。ここに逸脱を記録しつつ
該当 `F-NN` 行に触れておくことで、仕様が陳腐化するのを防げる。
...
## パイプライン連携（feature-pipeline / task-pipeline 共通）

このスキルは pipeline プラグインの feature-pipeline（コード）と task-pipeline（成果物）の
**両モード共通**で使う。以下は上記の基本ルールに**優先して**加わる、パイプライン内での
読み替えルール。
