...
1. スクリプトを実行する（**`--auto` は付けない** = 新規作成も行う）:
   ...
   ```
   - プラグイン導入時は `bash "${CLAUDE_PLUGIN_ROOT}/hooks/gen-agents-md.sh"`。
...
| スクリプトが見つからない | `gen-agents-md.sh` が配置されていない | プラグイン導入なら `${CLAUDE_PLUGIN_ROOT}/hooks/`、コピー導入なら `.claude/hooks/` に配置されているか確認する |
