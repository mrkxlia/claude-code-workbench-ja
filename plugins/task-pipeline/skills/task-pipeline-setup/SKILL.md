...
4. **opus が使える環境かを確認する。** `brief-writer` エージェントは既定で `model: opus` を使う
   （構成ミスが最も高くつく工程のため）。Sonnet のみの環境（会社 PC 等）ではこの指定のまま導入すると
   Phase 3 で失敗・意図しないフォールバックが起きうる。ユーザーに「opus は使えますか？」と確認し、
   使えない場合は Step 5 で `brief-writer.md` の `model: opus` を `model: inherit` に書き換える。
...
7. **opus 可用性（Step 0-4）** — 使える／使えない。使えない場合は「`brief-writer` を `inherit` に
   書き換えて導入します」と伝える
...
**opus が使えない環境の場合**（Step 0-4 で確認済み）: コピー後、`brief-writer.md` の
...
- [ ] opus が使えない環境と確認した場合、brief-writer.md の model が inherit に書き換わっている
