...
2. **workbench リポジトリ自身の中で実行していないこと**を確認する。
   ルートに `plugins/task-pipeline/` ディレクトリと `plugins/software-pipeline/` ディレクトリ（または
   `.claude-plugin/marketplace.json`）がある場合は workbench 本体なので、「導入先のプロジェクトで
   実行してください」と案内して停止する
...
4. **opus が使える環境かを確認する。** `brief-writer` エージェントは既定で `model: inherit`
   （構成ミスが最も高くつく工程だが、opus 契約の有無を問わず安全に動く既定値）。
...
パスを教えてもらう。パス直下に `plugins/task-pipeline/agents/` が存在することを確認して
テンプレートソース `$SRC=<パス>/plugins/task-pipeline` とする。
...
# $SRC="$TMP/workbench/plugins/task-pipeline"
```
...
# $SRC="$TMP/claude-code-workbench-ja-main/plugins/task-pipeline"
```
...
7. **opus 可用性（Step 0-4）** — 使える／使えない。使える場合は「`brief-writer` を `opus` に
...
🔓 非gitモード: ハードルール7「1 Todo = 1 Commit = 1 Spec Update（`spec-sync-reminder` フックが
更新漏れを促す）」は、git が無い環境では「Commit」の単位とフックの実発火が成立しないため、
...
1. `mkdir -p .claude/agents` して `$SRC/agents/*.md` の5ファイルをコピーする
...
**opus が使える環境の場合**（Step 0-4 で確認済み）: コピー後、`brief-writer.md` の
