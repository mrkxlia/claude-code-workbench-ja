...
| 症状 | 原因 | 対処 |
|---|---|---|
| 「codex CLI が見つかりません」／認証エラー | 未導入・未認証 | README の「前提」を案内する。実行を強行しない。急ぐなら本体の `/code-review` を提案する |
| レビュー対象が空 | 未コミット差分が無い／指定パスが存在しない | スコープを確認する（`uncommitted` → `base [branch]` への切り替え、パスの綴り）。空のまま Codex を起動しない |
| 重大度が構造化されていない | 構造化出力が使えず plain パスで実行された | 結果に「重大度は Codex の判断であり構造化出力ではない」と明記して提示する（フロー4の注記） |
| 応答が返らない・途中で切れる | 同梱した差分が大きすぎる | 「全文 → 関連抜粋 → diff → パス名指し」の順に降格する |
| 指摘がワークスペース外のファイルを指す | Codex が文脈を取り違えた | その指摘は落とし、対象スコープを明示して再依頼する |
| 指摘が的外れに多い | スコープが広すぎる | 対象を絞って再実行する。P3/P4 の大量指摘は要約にとどめ、判断をユーザーに委ねる |
