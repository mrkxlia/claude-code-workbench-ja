...
description: >-
  ...
  「このリポジトリはなぜこの構成なのか」「設計の意図を知りたい」にも応じ、設計判断の理由は
  ADR・PR・commit・コメント・人に聞いた答えのいずれかを出典として添える。
...
4. **分からないことは10章「未確認」に落とす。** それらしく埋めた1行は、読者を間違った実装に導く。
   空欄のまま残す方が安い
5. **「なぜ」は出典のあるものだけ書く。** 設計判断の理由（9章）はコードに書かれていないことが
   多い。ADR・PR・commit・コメント・人に聞いた答えのいずれも無いなら、**復元を試みずに
   10章へ落とす**。もっともらしい理由は、間違っていても読者に検証できない
...
10章「未確認」が埋まっているなら、[`references/interview.md`](references/interview.md) を読み、
既存メンバーに聞く質問に変換する。聞ける相手がいない場合はこの Step を飛ばす。

**9章「設計判断の理由」の空欄は、ここで埋まる分がいちばん多い。** 判断の理由はコードではなく
人の記憶にあることが多く、聞けば `（日付・誰に確認）` を出典として書ける。
...
**必須**: 「なぜ今の形なのか」を1件1行で、**出典つき**で。書けるのは次の5つから採ったものだけ。

| 書いてよい出典 | 添える根拠の形 |
|---|---|
| ADR・設計ドキュメント・RFC | `docs/adr/0007-session-store.md` |
| PR・issue の本文、レビューのやり取り | `#412` |
| 「なぜ」が書かれている commit メッセージ | `a1b2c3d` |
| コード・設定のコメントに明記された理由 | `src/db/pool.ts:31` |
| 人に聞いた答え | `（2026-09-06・田中さんに確認）` |

**この章に推測を書かない。** ライブラリ名や構成から理由を「復元」した文は、読者が検証できない
（この章を読むのは、たいてい判断の当事者ではない人である）。出典の無い理由は10章へ落とし、
Step 6 で聞く。**却下された代替案とその理由が見つかったら、それがこの章で最も価値が高い。**

- **悪い例**: 「パフォーマンスのために Redis を採用しています。」
  （出典が無く、事実なのか書き手の推測なのかを読者が区別できない）
- **良い例**: 「セッションを Redis に置いているのは、ECS のタスクが複数あり、アプリ内メモリでは
  ログイン状態がタスクをまたげないため（`#412`）。当初は DB のテーブルに持っていたが、ピーク時に
  DB の接続数を食い潰したため移した（`docs/adr/0007-session-store.md`）。DynamoDB は当時
  VPC エンドポイントを立てられず見送られた（同 ADR）。」

理由が1件も見つからなかった場合も**章は残し、探した範囲**（ADR ディレクトリの有無・
`git log` を追った期間・読んだ PR の範囲）を書く。章ごと省略すると、**探して無かったのか、
探していないのか**が読者に区別できない。
...
| 断定した記述に根拠があるか | 根拠を足すか、10章「未確認」へ移す |
| 9章の各行に出典（ADR・PR 番号・commit・`ファイル:行`・確認した人と日付）があるか | 出典の無い理由は10章へ移す。構成から推測した理由をここに残さない |
...
| 10章が空になっていないか | 空なら推測で埋めた疑いがある。断定した箇所を洗い直す |

## やらないこと

- 図を「文章で代替してよい」と判断すること（3・4・5章では認めない）
- ディレクトリ名・クラス名からの推測で章を埋めること
- **設計判断の理由を、採用技術や構成から推測して書くこと**（9章は出典のあるものだけ。
  出典が無ければ10章へ落とす。読みやすい「なぜ」ほど、間違っていても検証されずに残る）
