...
- **(A) Plan モードレビュー**: `EnterPlanMode` →（step 4 の要点サマリー＋story.md 全文 ＋ 遷移宣言
  「承認後は spec-writer を起動して技術ブリーフを作り、CP2 で必ず停止する」）を計画として提示 → `ExitPlanMode` で承認を得る。
...
修正指示があれば story-writer を修正内容つきで再起動し、再びここで停止する（再提示も step 4 と同じ要点サマリー＋全文の形式で。(A) の場合は再び `EnterPlanMode` に入り直して提示する）。
...
修正指示があれば spec-writer を再起動し、再びここで停止する（再提示も step 4 と同じ要点サマリー＋全文の形式で。(A) の場合は再び `EnterPlanMode` に入り直して提示する）。
