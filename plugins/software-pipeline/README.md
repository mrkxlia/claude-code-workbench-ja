...
プラグイン化されているのは**スキル7種とエージェント7種**です（いずれもプラグイン導入で自動配信）。
CLAUDE.md・フックはプロジェクトごとの差し替え（担当範囲など）が前提のため、プラグインからは
...
cp <このリポジトリ>/plugins/software-pipeline/agents/*.md .claude/agents/
```
...
cp -r <このリポジトリ>/plugins/software-pipeline/skills/feature-pipeline .claude/skills/
cp -r <このリポジトリ>/plugins/software-pipeline/skills/clarify .claude/skills/
cp -r <このリポジトリ>/plugins/software-pipeline/skills/build-with-tests .claude/skills/
cp -r <このリポジトリ>/plugins/software-pipeline/skills/notes .claude/skills/
cp -r <このリポジトリ>/plugins/software-pipeline/skills/spec-extract .claude/skills/
cp -r <このリポジトリ>/plugins/software-pipeline/skills/pipeline-improve .claude/skills/
```
...
cp <このリポジトリ>/plugins/software-pipeline/hooks/block-secrets-commit.sh .claude/hooks/
cp <このリポジトリ>/plugins/software-pipeline/hooks/guard-builder-writes.sh .claude/hooks/
...
```bash
cp <このリポジトリ>/plugins/software-pipeline/setup/settings.json .claude/settings.json
```
...
git clone --depth 1 https://github.com/mrkxlia/claude-code-workbench-ja /tmp/workbench
mkdir -p ~/.claude/skills
cp -r /tmp/workbench/plugins/software-pipeline/skills/clarify ~/.claude/skills/
cp -r /tmp/workbench/plugins/software-pipeline/skills/build-with-tests ~/.claude/skills/
```
...
- **Edit/Write のパス検査フック** — 「担当範囲」をプロンプトによる約束ではなく機械的に補強したい場合、
  ...
  [`guard-deliverable-writes.sh`](../task-pipeline/hooks/guard-deliverable-writes.sh)
...
成果物の種類）で**コードモード / 成果物モードを自動判定**するため、どちらの名前空間
（`/software-pipeline:clarify` / `/task-pipeline:clarify`）で解決されても挙動は同じです。
プロジェクトへ直接コピーした場合は短い名（`/clarify` 等）で呼べます。

> 補足: プラグインの同名スキルには、frontmatter `name` で名前空間 prefix が失われる既知バグ
> （[anthropics/claude-code#22063](https://github.com/anthropics/claude-code/issues/22063)）や
> 後から導入した側が上書きする報告があります。統合連携版は**どちらが選ばれても同一内容**なので、
> これらの問題が起きても実害がありません。
...
| `clarify` | 両方 | 統合連携版（両版同一） | モード自動判定で両パイプラインに対応 |
| `notes` | 両方 | 統合連携版（両版同一） | implementation-skills 原本＋統合連携セクション |
| `spec-extract` | 両方 | 統合連携版（両版同一） | implementation-skills 原本＋統合連携セクション |
