...
2. **workbench リポジトリ自身の中で実行していないこと**を確認する。
   ルートに `plugins/software-pipeline/` ディレクトリと `plugins/task-pipeline/` ディレクトリ（または
...
4. **opus が使える環境かを確認する。** `spec-writer` エージェントは既定で `model: inherit`
   （設計ミスが最も高くつく工程だが、opus 契約の有無を問わず安全に動く既定値）。
   ユーザーに「opus は使えますか？」と確認し、使える場合は Step 5 で `spec-writer.md` の
...
- workbench リポジトリの clone 内の `plugins/software-pipeline/skills/pipeline-setup/` から
  直接読み込まれている（この場合 `$SRC=<clone>/plugins/software-pipeline`）
...
パスを教えてもらう。パス直下に `plugins/software-pipeline/agents/` が存在することを確認して
テンプレートソース `$SRC=<パス>/plugins/software-pipeline` とする。
...
# $SRC="$TMP/workbench/plugins/software-pipeline"
```
...
# $SRC="$TMP/claude-code-workbench-ja-main/plugins/software-pipeline"
```
...
7. **opus 可用性（Step 0-4）** — 使える／使えない。使える場合は「`spec-writer` を `opus` に
...
1. `mkdir -p .claude/agents` して `$SRC/agents/*.md` の7ファイルをコピーする
...
**opus が使える環境の場合**（Step 0-4 で確認済み）: コピー後、`spec-writer.md` の
...
詳細は [`references/windows.md`](references/windows.md)。bash が使える Git Bash / WSL では上記どおり `.sh` を使う。）
...
- `.claude/settings.json` が**無い**場合: `$SRC/setup/settings.json` をそのままコピーする
...
- [ ] opus が使える環境と確認した場合、spec-writer.md の model が opus に書き換わっている
