...
- **反グループシンク機構（採用7つ）**: ①構造的ブラインド ②ブラインドスコアリング＋全員一致警告
  （「全員一致は最も危険な失敗モード」）③具体性ゲート（反例・矛盾箇所のない批判は破棄）
  ④ゴーストパネリスト検証（空回答・契約不履行は1回だけ再委譲、駄目なら「欠席」— 声を捏造しない）
  ⑤追従的収束の検出（理由なき全面譲歩を無効化）⑥**統制検証ゲート**（対象を見なくても書けた
  一般論＝物証の無い指摘を統合から外す。落とした件数と理由は開示する）⑦**ブラインド最終採点**
  （R3 で他者の結論を見せないまま最終スコアを取り直し、R1 と並べて出す。討論で分散が縮んだ場合は
  根拠による収束か同調かを譲歩履歴と併せて判断する）＋ファシリテーター私見の分離。
