...
- 隣の改善点を見つけたら、実装せずに提案として列挙する。作業中に見つけた既存バグ・性能懸念も実装せず報告に回す（依頼された挙動がそれ無しに成立しない場合だけ直す。この例外は改善提案には適用しない）。
- テストも依頼の範囲に限定する。依頼にある場合か、リポジトリがその種の変更にテストを置いている場合だけ、隣接テストと同じ規模で追加する。検証用のスクラッチ確認を恒久ファイルとして残さない。
